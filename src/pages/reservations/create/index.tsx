...
function ReservationCreatePage() {
  ...
  const formik = useFormik<ReservationInterface>({
    initialValues: {
      ...
      time_update: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="time_update" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Time Update
            </FormLabel>
            <DatePicker
              selected={formik.values?.time_update ? new Date(formik.values?.time_update) : null}
              onChange={(value: Date) => formik.setFieldValue('time_update', value)}
            />
          </FormControl>
